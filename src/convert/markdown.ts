import type { ConversationDetail, ImageAssetPart, Message, MessageContent } from '../types'
  ...
  switch (c.content_type) {
    case 'text': {
      ...
      if (msg.author.role === 'assistant' && recipient !== 'all') {
        // Canvas / 联网等工具调用载荷（多为 JSON）：MVP 整块折叠嵌入，不丢内容，P2 做精细还原
        return callout('example', `工具调用 → \`${recipient}\``, fence(raw), true)
      }
      return renderProse(raw)
    }
    case 'multimodal_text':
      return (c.parts ?? [])
        .map(p => (typeof p === 'string' ? renderProse(p) : renderAsset(p)))
        .filter(s => s.trim() !== '')
        .join('\n\n')
    case 'code':
      return fence(c.text ?? '', codeLanguage(c, recipient))
    case 'execution_output':
      return callout('note', '运行输出', fence(c.text ?? ''), true)
    case 'thoughts':
      return renderThoughts(c)
    default:
...
function renderProse(raw: string): string {
  return demoteHeadings(convertMath(stripCitationMarkers(raw)))
}

function renderThoughts(c: MessageContent): string | null {
  const blocks = (c.thoughts ?? [])
    .map(t => {
      const head = t.summary?.trim() ? `**${t.summary.trim()}**\n\n` : ''
      return head + renderProse(t.content ?? '')
    })
    .filter(s => s.trim() !== '')
...
function renderAsset(p: ImageAssetPart): string {
  const pointer = typeof p.asset_pointer === 'string' ? p.asset_pointer : ''
  const id = pointer.replace(/^[a-z_-]+:\/\//, '') || p.content_type
  const size = p.width && p.height ? `（${p.width}×${p.height}）` : ''
  return `> [!missing] 图片附件 \`${id}\`${size} — 附件下载在 P2 支持`
}
